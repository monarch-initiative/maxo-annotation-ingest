"""Command line interface for maxo-annotation-ingest."""
...
def callback(
    ...
    """maxo-annotation-ingest CLI."""
    if version:
        from maxo_annotation_ingest import __version__

        typer.echo(f"maxo-annotation-ingest version: {__version__}")
        raise typer.Exit()


@app.command()
def download(force: bool = typer.Option(False, help="Force download of data, even if it exists")):
    """Download data for maxo-annotation-ingest."""
    typer.echo("Downloading data for maxo-annotation-ingest...")
...
def transform(
    ...
    """Run the Koza transform for maxo-annotation-ingest."""
    typer.echo("Transforming data for maxo-annotation-ingest...")
