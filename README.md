...
| [Documentation](https://monarch-initiative.github.io/maxo-annotation-ingest) |

Transform of MAxO Annotations into Biolink Model compliant KGX
...
Download the data for the maxo_annotation_ingest transform:

```bash
poetry run maxo_annotation_ingest download
```

To run the Koza transform for maxo-annotation-ingest:

```bash
poetry run maxo_annotation_ingest transform
```

To see available options:

```bash
poetry run maxo_annotation_ingest download --help
# or
poetry run maxo_annotation_ingest transform --help
```
